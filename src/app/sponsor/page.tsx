...
const VARIATION_IDS = {
  500: "66YNIBSHEY3P4SQDVBLKSXZT",
  750: "JMEXOJQ3CX6ZSFVAUEQRQZ6W",
  1000: "JCLAFFZKQJBDNCJEYCWCEJ3S",
  1500: "5RDQFP2UCPISDUI5CQQWBHK",
  2000: "RM7ANE6TXLJVZ2QG22OLRZH3",
  custom: "FVDHJMKITEC76Z7WNCSOCYZT",
};
  ...
  useEffect(() => {
    const fetchItem = async () => {
      try {
        ...
        setItemInfo({
          name: object?.name ?? "SHPE UCF Sponsorship",
        });
      } catch (err) {
        console.error("Failed to fetch item info:", err);
        setItemInfo({ name: "SHPE UCF Sponsorship" });
      }
    };
    ...
    <main className="min-h-screen bg-white py-12 px-4 flex justify-center">
      <div className="max-w-3xl w-full rounded-3xl shadow-xl p-8">
        <h1 className="text-4xl sm:text-5xl font-semibold text-center text-[var(--shpe-yellow)] mb-6 drop-shadow-md">
          {itemInfo?.name || "SHPE UCF Sponsorship"}
        </h1>

        <p className="text-lg text-black text-center leading-relaxed mb-8">
          SHPE UCF invites sponsors to invest in our mission to empower Hispanic
          engineers and scientists. Your sponsorship helps fund workshops,
          outreach, scholarships, and events that shape future leaders in STEM.
