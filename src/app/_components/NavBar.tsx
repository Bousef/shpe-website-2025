...
export default function Navbar() {
  ...
  const { data: member, isLoading } = api.user.getCurrentMember.useQuery(undefined, {
  ...
  useEffect(() => {
    ...
    // remove the ?refetchUser=1 from the URL without reloading
  ...
  const logout = api.user.logout.useMutation({
    onSuccess: async () => {
      await utils.user.getCurrentMember.invalidate(); // refresh navbar state
      router.push("/"); // after logging out, send them back to home or login
    },
  });
